...
const getallemployees = async (req, res, next) => {
    try {
        const response = await employee.find().populate('personaldetails').sort({ name: -1 });
        res.json({ response});
    } catch (error) {
...
const getbyusernamestaticmethod = async (req, res, next) => {
    try {
        ...
        const  response= await employee.findByusername(username)
        res.json({ response});
    } catch (error) {
        res.status(500).json({ message: "An error occurred", error:error.message });
    }
...
const register = async(req,res)=>{
    const { name,designation,email,age,username,password,role,DOB,address,phoneNo } = req.body;
    try {
      const newUser = await employee.create({ 
        ...
        password,
        role,
        personaldetails: { DOB:DOB,address:address,phoneNo: phoneNo } 
     });
...
//for virtual method
  //  const user = new employee({ _id: '65cb295f981515362d555d46',username: 'lisa23' });
   // console.log(user.idandusername);

module.exports ={getallemployees,getbyrolequerymethod,getbyusernamestaticmethod,getemployees,show,register,update,delete1 }